...
export class BaMenuService {
    ...
    menuItems.forEach((item) => {
      ...
      if (item.selected) {
        ...
        if (this.translate.isLanguageEN == true) {
              this._currentMenuItem["title"] = this._currentMenuItem["titleEN"];
          }
          else {
              this._currentMenuItem["title"] = this._currentMenuItem["titleLC"];
          }
      }
  ...
  protected _selectItem(object:any):any {
    ...
    object.pselected = false;

    var sp = this.router.url.split("/");
    if (sp.length >= 3) {
      if (object.route.paths.length == 3) {
        if (sp[2] == object.route.paths[2]) {
          object.pselected = true;
        }
      }
    }
